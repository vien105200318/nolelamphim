...
async function HeroSection() {
  const newMovies = await getNewMovies().catch(() => ({ items: [] }));
  if (newMovies.items.length === 0) return <ErrorBox />;
  return (
    <section className="mt-6">
      <Reveal><HeroCarousel movies={newMovies.items.slice(0, 8)} /></Reveal>
    </section>
  );
}

async function SubteamSection() {
  const subteam = await getSubteam().catch(() => ({ items: [] }));
  if (subteam.items.length === 0) return null;
  return (
    <Reveal delay={80}>
      <section className="mt-10">
        <h2 className="text-sm font-semibold text-text-primary mb-4">Subteam</h2>
...
async function NewMoviesSection() {
  const newMovies = await getNewMovies().catch(() => ({ items: [] }));
  if (newMovies.items.length === 0) return null;
  return (
    <Reveal delay={120}>
      <section className="mt-10">
        <h2 className="text-sm font-semibold text-text-primary mb-4">Phim mới cập nhật</h2>
